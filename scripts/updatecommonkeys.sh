...
if [[ -e beachos.keystore ]]; then
  keytool -importkeystore -srckeystore beachos.keystore -destkeystore beachos.p12 -srcstoretype jks -deststoretype pkcs12
  openssl pkcs12 -in beachos.p12 -out beachos.pem -nodes && rm beachos.p12
  openssl x509 -in beachos.pem -out chromium.x509.pem
  openssl pkcs8 -in beachos.pem -out chromium.pk8 -outform DER -topk8 -nocrypt && rm beachos.pem
fi
