#!/bin/bash
# Update the local BeachOS F-Droid repository

SCRIPTPATH="$(cd "$(dirname "$0")";pwd -P)"
DATE=$(date +%F)
FDROIDREPOTMP=$(mktemp -d)
JOBID=$(curl -sI https://gitlab.com/beachos/beachos-fdroid-repo/-/jobs/artifacts/main/download?job=fdroid-repo | grep -iw Location: | cut -d / -f 8)
ANDROID_MK=Android.mk
FDROID_MK=fdroid-repo.mk

pushd $SCRIPTPATH/../../../prebuilts/beach/fdroid
pushd $FDROIDREPOTMP
curl -L https://gitlab.com/beachos/beachos-fdroid-repo/-/jobs/${JOBID}/artifacts/download -o artifacts.zip || exit 1
unzip artifacts.zip
popd
rm -rf repo
...

{
	...
	echo -e "    prebuilts/beach/fdroid/fallback-icon.png:\$(TARGET_COPY_OUT_PRODUCT)/fdroid/repo/fallback-icon.png \\"
} > $FDROID_MK

for f in $FILES; do
	{
		echo -e "    prebuilts/beach/fdroid/$f:\$(TARGET_COPY_OUT_PRODUCT)/fdroid/$f \\"
	} >> $FDROID_MK
done
