...
func init() {
	android.RegisterModuleType("beach_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = beachExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = beachExpandVariables(ctx, dep_file)
	...
	cmd := beachExpandVariables(ctx, String(g.properties.Cmd))
