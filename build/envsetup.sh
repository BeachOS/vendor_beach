function __print_beach_functions_help() {
cat <<EOF
Additional BeachOS functions:
- aospremote:      Add git remote for matching AOSP repository.
- beachremote:     Add git remote for BeachOS Gerrit Review.
...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if [[ "$target" =~ -(user|userdebug|eng)$ ]]; then
            ...
            # This is probably just the Beach model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi

            lunch beach_$target-$variant
        fi
    fi
    return $?
}
...
function beachremote()
{
    ...
    git remote rm beach 2> /dev/null
    ...
    local CALYX_USER=$(git config --get review.review.beachos.org.username)
    if [ -z "$CALYX_USER" ]
    then
        git remote add beach ssh://review.beachos.org:29418/$PFX$PROJECT
    else
        git remote add beach ssh://$CALYX_USER@review.beachos.org:29418/$PFX$PROJECT
    fi
    echo "Remote 'beach' created"
}
...
function lineageremote()
{
    ...
    if [ -f ".gitupstream-lineage" ]; then
        ...
        local PROJECT=$(echo $REMOTE | sed -e "s#BeachOS/##g; s#platform_#android_#g; s#vendor_#android_vendor_#g;")
...
function repopick() {
    T=$(gettop)
    $T/vendor/beach/build/tools/repopick.py $@
}
